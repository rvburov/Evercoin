...
class WalletSerializer(serializers.ModelSerializer):
    """Сериализатор для счета с дополнительными полями."""
    ...
    def validate_name(self, value):
        """Проверка уникальности названия счета."""
...
class WalletTransferSerializer(serializers.ModelSerializer):
    """Сериализатор для перевода между счетами."""
    ...
    from_wallet_icon = serializers.CharField(
        source='from_wallet.icon',
        read_only=True,
    )
    to_wallet_icon = serializers.CharField(
        source='to_wallet.icon',
        read_only=True,
    )
    from_wallet_color = serializers.CharField(
        source='from_wallet.color',
        read_only=True,
    )
    to_wallet_color = serializers.CharField(
        source='to_wallet.color',
        read_only=True,
    )
    ...
    def validate(self, data):
        """Валидация перевода: принадлежность счетов и достаточность средств."""
        user = self.context['request'].user
        ...
        if from_wallet == to_wallet:
            raise serializers.ValidationError(
                "Нельзя переводить средства на тот же счет"
            )
...
class WalletSummarySerializer(serializers.Serializer):
    """Сериализатор для сводной информации по счетам."""
...
class WalletOperationSerializer(serializers.ModelSerializer):
    """Сериализатор для операций счета с данными категории."""
